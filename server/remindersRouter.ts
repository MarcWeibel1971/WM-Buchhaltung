...
import {
  invoices, invoiceReminders, customers, companySettings, documents, organizations, userOrganizations,
} from "../drizzle/schema";
...
import { DEFAULT_REMINDER_POLICY, parseReminderPolicy, policyForReminderLevel, suggestReminderLevel, type ReminderLevel, type ReminderPolicy } from "../shared/reminderPolicy";
...
// ─── Mahn-Policy (Default) ────────────────────────────────────────────────
export const REMINDER_POLICY = DEFAULT_REMINDER_POLICY;
type Level = ReminderLevel;

async function getOrganizationReminderPolicy(orgId: number): Promise<ReminderPolicy> {
  const db = await getDb();
  if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  const [organization] = await db.select({ reminderPolicy: organizations.reminderPolicy })
    .from(organizations).where(eq(organizations.id, orgId)).limit(1);
  return parseReminderPolicy(organization?.reminderPolicy);
}
...
export const remindersRouter = router({
   ...
   */
  getPolicy: orgProcedure.query(({ ctx }) => getOrganizationReminderPolicy(ctx.organizationId)),

  updatePolicy: orgProcedure
    .input(z.object({
      level1: z.object({ minDaysOverdue: z.number().int().min(0).max(365), feeAmount: z.number().min(0).max(1000), gracePeriodDays: z.number().int().min(0).max(90), label: z.string().min(1).max(100) }),
      level2: z.object({ minDaysOverdue: z.number().int().min(0).max(365), feeAmount: z.number().min(0).max(1000), gracePeriodDays: z.number().int().min(0).max(90), label: z.string().min(1).max(100) }),
      level3: z.object({ minDaysOverdue: z.number().int().min(0).max(365), feeAmount: z.number().min(0).max(1000), gracePeriodDays: z.number().int().min(0).max(90), label: z.string().min(1).max(100) }),
    }).refine((policy) => policy.level1.minDaysOverdue <= policy.level2.minDaysOverdue && policy.level2.minDaysOverdue <= policy.level3.minDaysOverdue, { message: "Die Mahnstufen müssen zeitlich aufsteigend sein." }))
    .mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const [member] = await db.select({ role: userOrganizations.role }).from(userOrganizations)
        .where(and(eq(userOrganizations.userId, ctx.user.id), eq(userOrganizations.organizationId, ctx.organizationId))).limit(1);
      if (!member || (member.role !== "owner" && member.role !== "admin")) throw new TRPCError({ code: "FORBIDDEN", message: "Nur Owner oder Admins können die Mahnpolicy ändern." });
      await db.update(organizations).set({ reminderPolicy: JSON.stringify(input) }).where(eq(organizations.id, ctx.organizationId));
      return input;
    }),
  ...
  openPositions: orgProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const t = today();
      const policy = await getOrganizationReminderPolicy(ctx.organizationId);
      ...
      return rows.map(r => {
        ...
        const suggested = isOverdue ? suggestReminderLevel(daysOverdue, maxLevel, policy) : null;
        ...
        return {
          ...
          suggestedFee: suggested ? policyForReminderLevel(suggested, policy).feeAmount : 0,
        };
      });
    }),
  ...
  create: orgProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const policy = policyForReminderLevel(input.level as Level, await getOrganizationReminderPolicy(ctx.organizationId));
  ...
  generatePdf: orgProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        logger.warn({ err: e, invoiceId: invoice.id, reminderId: reminder.id }, "Reminder PDF document record could not be created");
      }
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.includeInvoicePdf && invoice.pdfS3Url) {
        try {
          ...
          logger.warn({ err: e, invoiceId: invoice.id, reminderId: reminder.id }, "Original invoice PDF could not be attached to reminder email");
        }
      }
