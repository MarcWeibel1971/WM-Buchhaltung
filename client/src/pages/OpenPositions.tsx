...
export default function OpenPositions() {
  ...
  const exportOpenItemsQuery = trpc.invoices.exportOpenItemsCsv.useQuery(undefined, { enabled: false });
  const handleExportOpenItems = async () => {
    const result = await exportOpenItemsQuery.refetch();
    if (!result.data) { toast.error("Offene-Posten-CSV konnte nicht erstellt werden"); return; }
    const blob = new Blob(["\uFEFF" + result.data.csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url; link.download = result.data.filename; link.click(); URL.revokeObjectURL(url);
    toast.success(`${result.data.count} offene Debitorenposten exportiert`);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button variant="outline" className="gap-2" onClick={handleExportOpenItems} disabled={exportOpenItemsQuery.isFetching}>
          <FileDown className="h-4 w-4" /> {exportOpenItemsQuery.isFetching ? "Exportiere..." : "Offene Posten CSV"}
        </Button>
      </div>
