...
export const settingsRouter = router({
  ...
  createBankAccount: orgProcedure
    .input(z.object({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db.select().from(accounts).where(and(eq(accounts.organizationId, ctx.organizationId), eq(accounts.number, input.accountNumber))).limit(1);
      let accountId: number;
      if (existing.length > 0) {
        // Account exists – check if already a bank account
        if (existing[0].isBankAccount) {
          const existingBA = await db.select().from(bankAccounts).where(and(eq(bankAccounts.organizationId, ctx.organizationId), eq(bankAccounts.accountId, existing[0].id))).limit(1);
  ...
  updateBankAccount: orgProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (data.name) {
        const [ba] = await db.select().from(bankAccounts).where(and(eq(bankAccounts.organizationId, ctx.organizationId), eq(bankAccounts.id, id))).limit(1);
        if (ba) {
          await db.update(accounts).set({ name: data.name }).where(and(eq(accounts.organizationId, ctx.organizationId), eq(accounts.id, ba.accountId)));
        }
      }
      return { success: true };
    }),

  deleteBankAccount: orgProcedure
    .input(z.object({ id: z.number().int() }))
    .mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      // Get the bank account (org-scoped)
      const [ba] = await db.select().from(bankAccounts).where(and(eq(bankAccounts.organizationId, ctx.organizationId), eq(bankAccounts.id, input.id))).limit(1);
      ...
      // Delete bank account
      await db.delete(bankAccounts).where(and(eq(bankAccounts.organizationId, ctx.organizationId), eq(bankAccounts.id, input.id)));
      if (Number(jlCount?.count) === 0) {
        // Also delete the account from Kontenplan if no journal lines
        await db.update(accounts).set({ isBankAccount: false }).where(and(eq(accounts.organizationId, ctx.organizationId), eq(accounts.id, ba.accountId)));
        await db.delete(accounts).where(and(eq(accounts.organizationId, ctx.organizationId), eq(accounts.id, ba.accountId)));
  ...
  updateAccount: orgProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.isBankAccount === true && !cleanData.category) {
        const [existing] = await db.select().from(accounts).where(and(eq(accounts.organizationId, ctx.organizationId), eq(accounts.id, id))).limit(1);
